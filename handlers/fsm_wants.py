...
class FSMAdminWants(StatesGroup):
    info = State()
    username = State()
    amount = State()
    submit = State()
...
async def fsm_start(message: types.Message):
    ...
    if result is None:
        ...
        await FSMAdminWants.info.set()
        available = await DataBase.get_user_wants(f"@{message.from_user.username}")
        if available["available"] <= 0:
            await message.answer("У вас не осталось днег", reply_markup=cancel_markup)
...
async def submit(message: types.Message, state: FSMContext):
    if message.text == "ДА":
        await DataBase.insert_wants(state)
        await state.finish()
        await message.answer(
...
def register_handlers_fsm_wants(dp: Dispatcher):
    dp.register_message_handler(cancel_reg, state="*", commands=["CANCEL"])
    dp.register_message_handler(
        cancel_reg, Text(equals=["Отмена", "Отмена"], ignore_case=True), state=["*"]
    )
    dp.register_message_handler(fsm_start, commands=["wants"])
    dp.register_message_handler(load_info, state=FSMAdminWants.info)
    dp.register_message_handler(load_amount, state=FSMAdminWants.amount)
    dp.register_message_handler(submit, state=FSMAdminWants.submit)